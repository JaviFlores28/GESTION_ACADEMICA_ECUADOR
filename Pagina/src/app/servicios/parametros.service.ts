...
export class ParametrosService extends ErrorHandlerService {
  private apiUrl = variables.URL_API + '/parametro'; // Reemplaza con tu URL
  ...
  post(parametro: Parametro): Observable<any> {
    return this.http.post(this.apiUrl, parametro).pipe(catchError(this.handleError));
  }

  put(parametro: Parametro): Observable<any> {
    return this.http.put(this.apiUrl, parametro).pipe(catchError(this.handleError));
  }
