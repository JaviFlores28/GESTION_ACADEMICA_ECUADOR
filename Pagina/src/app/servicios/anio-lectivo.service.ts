...
export class AnioLectivoService extends ErrorHandlerService {
  ...
  post(anioLectivo: AnioLectivo): Observable<any> {
    return this.http.post(this.apiUrl, anioLectivo).pipe(catchError(this.handleError));
  }

  put(anioLectivo: AnioLectivo): Observable<any> {
    return this.http.put(this.apiUrl, anioLectivo).pipe(catchError(this.handleError));
  }
