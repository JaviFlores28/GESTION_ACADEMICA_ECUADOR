...
class Curso {
  CRS_ID: string;
    ...
    constructor(CRS_ID: string, CRS_NOM: string, CRS_TIPO: string, CRS_ORDEN: number, ESTADO: number, CREADOR_ID: string, FECHA_CREACION?: Date | undefined) {
       this.CRS_ID = CRS_ID;
...
isValid(): boolean {
  return !!this.CRS_ID && !!this.CRS_NOM && !!this.CRS_TIPO && !!this.CRS_ORDEN && !!this.CREADOR_ID;
}
