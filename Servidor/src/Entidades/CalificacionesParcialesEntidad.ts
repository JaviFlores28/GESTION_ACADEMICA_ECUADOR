...
class CalificacionesParciales {
  CAL_ID: string;
    ...
    constructor(CAL_ID: string, PRF_ASG_PRLL_ID: string, PRLL_EST_ID: string, PRCL_ID: string, CALIFICACION: number, ESTADO: number, CREADOR_ID: string, FECHA_CREACION?: Date | undefined) {
       this.CAL_ID = CAL_ID;
...
isValid(): boolean {
  return !!this.CAL_ID && !!this.PRF_ASG_PRLL_ID && !!this.PRLL_EST_ID && !!this.PRCL_ID && !!this.CALIFICACION && !!this.CREADOR_ID;
}
