...
router.post('/aniolectivo', async (req, res) => {
   try {
    ...
    const anio_lectivo = new AnioLectivoEntidad(request.AL_ID, request.AL_NOM, request.AL_INICIO, request.AL_FIN, request.AL_POR_PRD, request.AL_POR_EXAM, request.CLFN__MIN_APR, request.CLFN__MIN_PERD, request.NUM_PRD, request.NUM_EXAM, request.NUM_PRCL, request.NUM_SUSP, request.ESTADO, request.CREADOR_ID, request.FECHA_CREACION);
